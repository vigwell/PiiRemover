...
public class RedactController : ControllerBase
{
    ...
    public async Task<IActionResult> Redact(IFormFile file, CancellationToken ct)
    {
        ...
        try
        {
            ...
            _ = FireWebhookAsync(clientId, file.FileName, result.Matches.Count, fieldsHit, result.DurationMs);
    ...
    private async Task FireWebhookAsync(int? clientId, string? fileName, int matchCount, string[] fieldsHit, long durationMs)
    {
        ...
        try
        {
            ...
            if (client?.WebhookUrl is null or { Length: 0 }) return;
        ...
        catch { /* webhook failures must never affect the API response */ }
    }
