...
public class VideoSettings
{
    ...
    public const string KeyDefaultCreateCaptions      = "video:defaultCreateCaptions";
    ...
    public static readonly Dictionary<string, (string Default, string Description)> Metadata = new()
    {
        ...
        [KeyDefaultCreateCaptions]      = ("true",  "Default value for the createCaptions per-request flag. When true, the Processing Options panel pre-checks 📄 Create Captions. Generates a WebVTT sidecar .vtt file from the transcript text — the MP4 video itself is never altered."),
        [KeyPiiRedactionEnabled]        = ("true",  "Default value for the redactPii per-request flag. When true, the Processing Options panel pre-checks 🔒 Redact PII in Captions. Replaces PII matches with █ in the .vtt captions file. Only applies when createCaptions is also true."),
        [KeyPiiAudioRedactionEnabled]   = ("true",  "Default value for the redactAudioPii per-request flag. When true, the Processing Options panel pre-checks 🔇 Silence PII in Audio. Runs offline speech recognition to locate PII words and mutes them in the output MP4 via FFmpeg. Requires audio file to be provided."),
    ...
    public async Task<bool>   GetDefaultCreateCaptionsAsync()     =>  (await GetAsync(KeyDefaultCreateCaptions)).Equals("true", StringComparison.OrdinalIgnoreCase);
