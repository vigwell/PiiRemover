...
public class TesterModel : AdminPageModel
{
    ...
    public async Task<IActionResult> OnGetFieldsAsync()
    {
        ...
        return new JsonResult(fields.Select(f => new
        {
            ...
            patterns    = f.Patterns.Select(p => new
            {
                ...
                pattern           = p.Pattern,
                priority          = p.Priority,
                scopeEndPosition  = p.ScopeEndPosition
            }).ToArray()
        }).ToArray());
    }
    ...
    public async Task<IActionResult> OnPostCreateFieldAsync([FromBody] CreateFieldRequest req)
    {
        ...
        var rawRw    = req.ReplaceWith?.Trim() ?? string.Empty;
        var rw       = req.IsPreserve ? "—"
                     : (rawRw.Length > 0 ? rawRw[0].ToString() : "█");
    ...
    // ── Update ScopeEndPosition for a FileList pattern (called from Live Tester) ─
    public async Task<IActionResult> OnPostSetScopeEndPositionAsync([FromBody] SetScopeEndRequest req)
    {
        var all     = await _fields.GetAllFieldsAsync();
        var pattern = all.SelectMany(f => f.Patterns).FirstOrDefault(p => p.Id == req.PatternId);
        if (pattern is null)
            return new JsonResult(new { ok = false, error = "Pattern not found." });

        await _fields.UpdatePatternAsync(req.PatternId, pattern.PatternType, pattern.Pattern,
            pattern.Priority, pattern.ScopeStart, pattern.ScopeEnd, req.Position);
        _fieldsCache.Invalidate();
