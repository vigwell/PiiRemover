...
builder.Services.AddSingleton<IAiService>(sp => sp.GetRequiredService<OllamaService>());
builder.Services.AddSingleton<IPatternEngine>(sp =>
    new LlmPromptEngine(sp.GetRequiredService<IAiService>()));
...
// ── HTTP client (used for webhook callbacks) ──────────────────────────────────
...
#if DEBUG
{
    ...
    var aiUrl     = await aiRepo.GetAsync("ai:baseUrl")        ?? cfg["Ollama:BaseUrl"]        ?? "http://localhost:11434";
