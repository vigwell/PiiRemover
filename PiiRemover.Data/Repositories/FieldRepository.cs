...
public class FieldRepository : IFieldRepository
{
    ...
    public async Task UpdatePatternAsync(int patternId, PatternType type, string pattern, int priority,
        string? scopeStart = null, string? scopeEnd = null, int scopeEndPosition = 500)
    {
        using var conn = Open();
        await conn.ExecuteAsync(
            "UPDATE PiiPatterns SET PatternType = @type, Pattern = @pattern, Priority = @priority, ScopeStart = @scopeStart, ScopeEnd = @scopeEnd, ScopeEndPosition = @scopeEndPosition WHERE Id = @patternId",
            new { patternId, type = type.ToString(), pattern, priority, scopeStart, scopeEnd, scopeEndPosition });
    }
