...
public class ClientRepository : IClientRepository
{
    ...
    public async Task UpdateWebhookUrlAsync(int id, string? webhookUrl)
    {
        using var conn = Open();
        await conn.ExecuteAsync("UPDATE Clients SET WebhookUrl = @webhookUrl WHERE Id = @id", new { webhookUrl, id });
    }
}
